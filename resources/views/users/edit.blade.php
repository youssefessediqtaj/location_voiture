...
<div class="max-w-xl mx-auto py-8">
    <div class="bg-white shadow rounded-lg p-6">
        <h2 class="text-2xl font-bold mb-4">Éditer un utilisateur</h2>
            ...
            <div class="mb-4">
                ...
                <select id="roles" name="roles[]" class="mt-1 block w-full border-gray-300 rounded" multiple>
                    @foreach($roles as $role)
                        <option value="{{ $role->id }}" {{ ($user->roles->pluck('id')->contains($role->id) || collect(old('roles'))->contains($role->id)) ? 'selected' : '' }}>{{ $role->name }}</option>
                    @endforeach
                </select>
